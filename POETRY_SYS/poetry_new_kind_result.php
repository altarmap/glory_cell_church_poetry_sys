...
if ($login_PRVL[PRVL_ID] == 2 || $login_PRVL[PRVL_ID] == 1){
 function insertdata ($aa,$bb)
   {
   return "insert into POETRY_KIND ($aa) values ($bb)";
}
$insert_KIND_info= insertdata ("KIND_NM,CRT_USER_NM,CRT_DT","\"".$SONG_KIND."\",\"".$login_PRVL[SEQ_NBR]."\",\"".$CRT_day."\"");
if(!mysql_query($insert_KIND_info,$MyLink)){
	header("Location:new_kind_erro.php");
	}
	else{
	header("Location:new_kind_successful.php");
}
}
	else{
